...
class Function(object):
    ...
    def v(self):
        return self.x2 - (0.3 + 0.6*(self.u()**2)*((1-self.u())**0.5) - 0.2*self.u())
    
    def a(self):
        return -5 + 26*(self.u()**2)*((1+self.u())**0.5) + 3*self.u()

    def b(self):
        return 40*(self.v()**2)*(1-self.v())/(1+10*(self.u()**2))
...
if __name__ == "__main__":
    ...
    func = Function()
    x0 = np.array([0.8, 0.3]).reshape(2, 1)
    e = 1e-8
    ...
    x2 = np.arange(0, 1.1, 0.001)
    X1, X2 = np.meshgrid(x1, x2)
    Z = func.f_value(np.array([X1, X2]))
    N = np.array([i*i*i*0.0001 for i in range(-500,500)])
